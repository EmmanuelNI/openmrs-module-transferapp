...
public class HibernateNeonatalTransferDao implements NeonatalTransferDao {
	...
	@Override
	public NeonatalTransfer saveNeonatalTransfer(NeonatalTransfer neonatalTransfer) {
		getSession().saveOrUpdate(neonatalTransfer);
		getSession().flush();
		return neonatalTransfer;
	}

	@Override
	@SuppressWarnings("unchecked")
	public List<NeonatalTransfer> getNeonatalTransfersByPatient(Patient patient) {
		return getNeonatalTransfersByPatient(patient, null);
	}

	@Override
	@SuppressWarnings("unchecked")
	public List<NeonatalTransfer> getNeonatalTransfersByPatient(Patient patient, Integer limit) {
		Criteria criteria = getSession().createCriteria(NeonatalTransfer.class);
	...
	@Override
	@SuppressWarnings("unchecked")
	public List<NeonatalTransfer> getOutboundNeonatalTransfersBySendingFacility(String sendingFacility,
			Integer patientId, Integer limit) {
		return getOutboundNeonatalTransfersBySendingFacility(sendingFacility, patientId, limit, null, null, null);
	}
	...
	@Override
	public int countOutboundNeonatalTransfers(String sendingFacility, Date fromDate) {
		if (StringUtils.isBlank(sendingFacility)) {
			return 0;
		}
	...
	public NeonatalTransfer getNeonatalTransferByUuid(String uuid) {
		...
		criteria.add(Restrictions.eq("voided", false));
		return (NeonatalTransfer) criteria.uniqueResult();
	}
